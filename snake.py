...
class Game(ctk.CTk):
    ...
    def get_input(self, event):
        match event.keysym:
            case 'Left': self.direction = DIRECTIONS['left'] if self.direction != DIRECTIONS['right'] else self.direction
            case 'Up': self.direction = DIRECTIONS['up'] if self.direction != DIRECTIONS['down'] else self.direction
            case 'Right': self.direction = DIRECTIONS['right'] if self.direction != DIRECTIONS['left'] else self.direction
            case 'Down': self.direction = DIRECTIONS['down'] if self.direction != DIRECTIONS['up'] else self.direction
